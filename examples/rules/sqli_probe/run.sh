...
echo "1> lint rule"
"$WFL_BIN" lint rules/sqli_probe.wfl --schemas "schemas/*.wfs"

echo "2> run inline tests"
TEST_OUT="$("$WFL_BIN" test rules/sqli_probe.wfl --schemas "schemas/*.wfs")"
echo "$TEST_OUT"
...
if ! grep -q '"__wfu_rule_name":"sqli_probe"' "$ALERT_FILE"; then
    echo "ERROR: missing sqli_probe alert" >&2
    cat "$ALERT_FILE" >&2
    exit 1
fi
...
echo "OK: sqli_probe produced 1 expected alert"
