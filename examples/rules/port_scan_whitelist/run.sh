...
echo "1> lint rule"
"$WFL_BIN" lint rules/port_scan_whitelist.wfl --schemas "schemas/*.wfs"

echo "2> run inline tests"
TEST_OUT="$("$WFL_BIN" test rules/port_scan_whitelist.wfl --schemas "schemas/*.wfs")"
echo "$TEST_OUT"
...
if ! grep -q '"__wfu_rule_name":"port_scan_whitelist"' "$ALERT_FILE"; then
    echo "ERROR: missing port_scan_whitelist alert" >&2
    cat "$ALERT_FILE" >&2
    exit 1
fi

if ! grep -q '"alert_type":"port_scan"' "$ALERT_FILE"; then
    echo "ERROR: missing port_scan alert_type" >&2
    cat "$ALERT_FILE" >&2
    exit 1
fi
...
echo "OK: port_scan_whitelist produced 1 expected alert"
