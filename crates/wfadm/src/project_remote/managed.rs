...
pub(super) fn backup_managed_dirs(work_root: &Path, dirs: &[&str]) -> Result<(), String> {
...
pub(super) fn restore_managed_dirs(work_root: &Path, dirs: &[&str]) -> Result<(), String> {
    let manifest_path = work_root.join(BACKUP_MANIFEST_PATH);
...
fn read_dir_names(path: &Path) -> Result<BTreeSet<String>, String> {
...
fn file_bytes_equal(left: &Path, right: &Path) -> Result<bool, String> {
...
fn non_utf8_path_err(path: impl Into<String>) -> String {
    format!("non-utf8 path under {}", path.into())
}

fn unsupported_file_type_under_err(path: impl Into<String>) -> String {
    format!("unsupported file type under {}", path.into())
}

fn unsupported_file_type_err(path: impl Into<String>) -> String {
    format!("unsupported file type {}", path.into())
}

fn symlink_copy_unsupported_err(path: impl Into<String>) -> String {
    format!(
