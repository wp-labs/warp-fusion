...
- 可同时运行 **1000 条**检测规则
- 实时吞吐（NEXMark **单查询**口径，Linux 8 核，blackhole 汇，2026-08-27）：**200 万 – 3100 万 EPS**
- 窗口数据按**分钟级**保留
- 事件缓冲约 **400 万条**（400 个窗口 × 每窗口 1 万条，单条事件约 1KB）

> ⚠ **两条引用纪律**：
> ① 本节早期曾写「实时处理万级事件（约 1 万 EPS）」，**已作废**；
> ② 上表为 **blackhole 汇、不落告警**的纯算力口径。历史上另有一次多规则混合负载测量（2026-08-10），**已过时且未复测，不得引用**。告警密集场景请按自身告警率实测。
...
| 许可与托管 | 开源免费（Apache 2.0）；托管服务（Confluent / Kinesis）按小时计费 | **ELv2（source-available，非 OSI 开源）**：企业内部自用免费（含部署 / 修改 / 嵌入自有产品）；作为托管服务对外提供需商业授权 |

~~**规则编写成本是差距最大的单项**~~ → **该论证已被 AI 辅助编码推翻**（2026-08-30 修正）：

- **编写**环节：AI 对 Flink 与 WFL 同等加速，**已不再是差异点**；
- 差距后移到**验证与上线**：WFL 规则内联 `test`，一键验证命中/未命中 + 热加载即时生效；Flink 即便 AI 秒出代码，仍需起环境、造数据、编译、部署 job 才能确认对错。**AI 让"写出来"变快，没让"验对"变快**；
- ⚠ **旧说法（AI 出现前，已作废）**：~~"规则编写成本是差距最大项"~~。
- **新论证**：判断语言对 AI 是否友好，看**表面积 / 语义明确度 / 验证反馈速度**三项——WFL 几行声明式、
  语义显式、`wfl test` 秒级验证，**规则可完全交由 AI 生成**（实践中已如此），人只做审核验收；
  Flink 侧即便 AI 秒出代码，仍需起环境/造数据/部署 job 才能确认对错，且隐式约定多、出错面大。
- **守住边界**：**AI 生成 ≠ 免验证**。检测规则出错是静默漏报/误报，产出必须跑 `wfl test` 并纳入回归。
  支撑设施：`wf-skills` 技能包（6 个技能装进 Claude/Codex）+ `wf-rules` 语料 + LSP 即时校验。
...
| 规模 | **吞吐不是瓶颈**（8 核单查询实测 200 万–3100 万 EPS）；真正的边界是**多节点高可用与分布式容错**——无 exactly-once / checkpoint |
| 生态与信任 | 十年生产验证、数百个社区 connector（真实护城河）；WarpFusion 核心运行时 2.0.8 / CLI 0.5.3，仍在快速演进——**但它已有基于注册表的 connector 体系**（可外部扩展），`wp-connectors` 覆盖 Kafka、MySQL、PostgreSQL、Doris、Elasticsearch、ClickHouse、VictoriaLogs、VictoriaMetrics、Prometheus、HTTP、UDP 及 file / tcp / syslog 等主流通道，差距在**广度**（尤其 SaaS 与小众数据源）而非有无 |
