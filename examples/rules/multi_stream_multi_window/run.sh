...
echo "1> lint rule"
"$WFL_BIN" lint rules/multi_stream_multi_window.wfl --schemas "schemas/*.wfs"

echo "2> run inline tests"
TEST_OUT="$("$WFL_BIN" test rules/multi_stream_multi_window.wfl --schemas "schemas/*.wfs")"
echo "$TEST_OUT"
...
if ! grep -q '"__wfu_rule_name":"dns_tunnel_probe"' "$ALERT_FILE"; then
    echo "ERROR: missing dns_tunnel_probe alert" >&2
    cat "$ALERT_FILE" >&2
    exit 1
fi
...
echo "OK: multi-stream dynamic routing produced 2 expected alerts"
