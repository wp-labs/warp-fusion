...
echo "1> lint rules"
for rule in rules/*.wfl; do
    "$WFL_BIN" lint "$rule" --schemas "schemas/*.wfs"
done

echo "2> run inline tests"
for rule in rules/*.wfl; do
    echo "==> $rule"
    TEST_OUT="$("$WFL_BIN" test "$rule" --schemas "schemas/*.wfs")"
    echo "$TEST_OUT"
    if echo "$TEST_OUT" | grep -q '^FAIL[[:space:]]'; then
        echo "ERROR: inline tests failed: $rule" >&2
        exit 1
    fi
done
...
if [ "$ALERT_COUNT" != "0" ]; then
    echo "ERROR: expected 0 alerts, got $ALERT_COUNT" >&2
    cat "$ALERT_FILE" >&2
    exit 1
fi
...
echo "OK: rat_propagation batch replay produced 0 alerts"
