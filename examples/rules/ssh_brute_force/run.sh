...
echo "1> lint rule"
"$WFL_BIN" lint rules/ssh_brute_force.wfl --schemas "schemas/*.wfs"

echo "2> run inline tests"
TEST_OUT="$("$WFL_BIN" test rules/ssh_brute_force.wfl --schemas "schemas/*.wfs")"
echo "$TEST_OUT"
...
if ! grep -q '"alert_type":"ssh_brute_force"' "$ALERT_FILE"; then
    echo "ERROR: missing ssh_brute_force alert_type" >&2
    cat "$ALERT_FILE" >&2
    exit 1
fi

if ! grep -q '"evidence_event_ids":\["auth-001"' "$ALERT_FILE"; then
    echo "ERROR: missing evidence_event_ids output" >&2
    cat "$ALERT_FILE" >&2
    exit 1
fi
...
echo "OK: ssh_brute_force produced 1 expected alert"
