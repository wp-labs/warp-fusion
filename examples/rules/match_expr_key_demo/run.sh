...
echo "1> lint rule"
"$WFL_BIN" lint rules/match_expr_key_demo.wfl --schemas "schemas/*.wfs"

echo "2> run inline tests"
TEST_OUT="$("$WFL_BIN" test rules/match_expr_key_demo.wfl --schemas "schemas/*.wfs")"
echo "$TEST_OUT"
...
if [ "$ALERT_COUNT" != "5" ]; then
    echo "ERROR: expected 5 alerts, got $ALERT_COUNT" >&2
    cat "$ALERT_FILE" >&2
    exit 1
fi

# 表达式派生 key（issue #80）：5 行 → 5 个不同派生组，逐组断言。
# ip 命中 / 用户回退 / 主机回退 / 全缺兜底 unknown（target 侧同理）。
for kv in '"attacker_key":"10.0.0.8"' '"attacker_key":"alice"' '"attacker_key":"bob-ws"' '"attacker_key":"unknown"' '"target_key":"db-02"' '"target_key":"unknown"'; do
...
echo "OK: match_expr_key_demo produced 5 alerts (coalesce-normalized entity group keys, issue #80)"
